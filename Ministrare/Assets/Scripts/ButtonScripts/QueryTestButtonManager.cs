...
public class QueryTestButtonManager : MonoBehaviour {
    ...
    // Reference to our Query Dialog Runner
    public QueryDialogRunner queryRunner;

	// Use this for initialization
	void Start () {
        state = buttonState.basic;
        opOne = new string[] { "test convo", "test convo 2"};
    ...
    void Init()
    {
        if (initialized == false)
        {
            ...
            for(int x = 0; x < 1; x++)
            {
                GameObject ob = CreateButton("test topic", ops[x]);
                buttonList[x] = ob;
                if (x % 2 == 0)
                {
                    pos.x += 200;
    ...
    GameObject createSelectionButton(string name)
    {
        ...
        button.onClick.AddListener(() => MS1SelectConvo(name));
    ...
    void MS1SelectConvo(string convoName)
    {
        queryRunner.GoToTestConvoWithName(convoName);
    }
